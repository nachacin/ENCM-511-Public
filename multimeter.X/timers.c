...
void start_T2(uint16_t delay_ms){
    
    int equivalent_ticks;       // declare equivalent_ticks
    uint16_t COSC = OSCCONbits.COSC;
    
    // Determine equivalent_ticks based on clock
    /* numbers come from simplifying the following function: 
     * ((Fosc/256)/2)*(delay_ms/1000); 
     * this equation takes into account the 256 pre-scaler, the Fosc/2, and
     * the ms to s conversion */  
    ...
    switch(COSC) {
        case 0:
            equivalent_ticks = 16 * delay_ms;
            break;
        case 5:
            equivalent_ticks = delay_ms / 17;
            break;
        case 6:
